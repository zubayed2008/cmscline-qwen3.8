...
/**
 * GET /api/accounting/ap-aging?asOf=YYYY-MM-DD (defaults to today)
 * Accounts Payable aging buckets (Current / 1-30 / 31-60 / 61-90 / 90+).
 */
export async function GET(request: NextRequest) {
  try {
    ...
    return successResponse(await LedgerService.apAging(parsed.asOf));
  } catch (error) {
