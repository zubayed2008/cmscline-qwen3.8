...
export default function VendorForm({ initialData }: VendorFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push('/admin/accounting/vendors');
      router.refresh();
    } catch {
    ...
    <Card className="max-w-3xl">
      <CardHeader>
        <h2 className="text-lg font-semibold text-gray-900">
          {isEditing ? `Edit Vendor ${initialData.code}` : 'Create New Vendor'}
        </h2>
      </CardHeader>
      <CardBody>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-4">
            ...
            <Button type="button" variant="secondary" onClick={() => router.push('/admin/accounting/vendors')}>
              Cancel
            </Button>
          </div>
        </form>
