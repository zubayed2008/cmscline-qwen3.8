...
> - **Part 5 — Financial Reporting: COMPLETE** — commit **pending** (not committed — awaiting user instruction): `LedgerService` (`getGeneralLedger` running-balance + pagination, `trialBalance`, `profitLoss`, `balanceSheet` with current-year net-income equity plug + imbalance warning, `arAging`/`apAging` CASE-WHEN buckets Current/1-30/31-60/61-90/90+); report types in `accounting-types.ts` (ILedgerRow/ITrialBalance/ITrialBalanceRow/IProfitLoss/IProfitLossRow/IBalanceSheet/IBalanceSheetSection/IAgingReport/IAgingBucket/AgingBucketKey); `ledgerQuerySchema` + `agingQuerySchema`; six GET routes `/api/accounting/{ledger,trial-balance,profit-loss,balance-sheet,ar-aging,ap-aging}`. **Critical reporting invariant:** reports include `journal_entries.status IN ('POSTED','REVERSED')` because reversals are stored as POSTED (original flipped to REVERSED) — including both makes reversed pairs net to zero. `ledger-service.test.ts` **deferred** per user instruction ("do not perform unit test cases for now") — ask for it later
> - Gates green at Part 5: `tsc --noEmit` ✓ · ESLint ✓ (0 errors on touched files) · **278/278 tests / 16 suites** ✓ · production build ✓
> - **Part 6 — Admin UI: COMPLETE** — commit **pending** (not committed — awaiting user instruction): full admin module under `src/app/admin/(dashboard)/accounting/**` (dashboard w/ Cash/Bank·AR·AP·Tax Payable cards + recent JEs + quick links; accounts list/new/edit + toggle; journal-entries list/new/detail with state-machine actions Submit/Approve/Post/Reverse-with-reason/Delete-draft; invoices list/new/detail + `RecordPaymentModal` (shared customer/vendor, allocations); bills list/new/detail + approve/post/cancel + vendor payment; customers & vendors list/new/edit with status toggle; periods list + seed + audited Close/Reopen; tabbed Reports page — Trial Balance / P&L / Balance Sheet / AR Aging / AP Aging / General Ledger w/ paging); shared `StatusBadge`/`MoneyDisplay`/`ErrorBanner` in `src/components/features/admin/accounting/`; AdminSidebar gained the "Financials" nav group (9 links). Server pages fetch via services with `fetchAccounting` guard (friendly banner when PG is unreachable); client forms/actions hit the existing API routes
> - Gates green at Part 6: `tsc --noEmit` ✓ · ESLint ✓ (0 problems on all new UI files; fixed React 19 `set-state-in-effect` violations) · **278/278 tests / 16 suites** ✓ · production build ✓ (21 new `/admin/accounting/**` routes registered)
...
> - Deferred manual gate (needs Docker Desktop): compose up → `npm run db:accounting:create` → `npm run db:accounting:migrate` → seed → exercise AR/AP/report flows live in the admin UI
> - **Next: Part 7 — Hardening & Docs** (live verification — stale-version update rejected, idempotency-key replay, forced mid-transaction rollback; `docs/accounting/ACCOUNTING-API.md`; spec §32 acceptance sweep; memory/future-plan status)
...
**Part 5 — Financial Reporting — ✅ COMPLETE (commit pending, NOT committed):** `LedgerService` — General Ledger (running balance + pagination), Trial Balance, P&L, Balance Sheet (current-year net-income equity plug + imbalance warning), AR/AP aging (CASE-WHEN buckets Current/1-30/31-60/61-90/90+) — aggregating `status IN ('POSTED','REVERSED')` entries only (reversal pairs net to zero); six report GET routes under `/api/accounting/`; `ledger-service.test.ts` skipped and asked for later (user: no unit tests for now)

**Part 6 — Admin UI — ✅ COMPLETE (commit pending, NOT committed):** every screen from the Admin UI section (dashboard, accounts list/new/edit, journal entries with state-machine actions, invoices + RecordPaymentModal, bills, customers, vendors, tabbed Reports, Periods close/reopen) plus shared `StatusBadge`/`MoneyDisplay`/`ErrorBanner` and the AdminSidebar "Financials" group
